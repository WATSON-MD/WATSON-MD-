...
async function syncdb() {
  ...
  let AstaConn = AstaConnectSock({
    ...
    browser: ["Windows", "chrome", "WATSON-MD"],
  ...
  AstaConn.ev.on("connection.update", async (_0x3fd0b3) => {
    ...
    if (_0x2224a7 === "open") {
      ...
      let _0x1f88ec =
        ...
        Config.WORKTYPE +
        "\n  Database: " +
        (isMongodb ? "MongoDb" : sqldb ? "PostegreSql" : "WATSON-MD") +
        "\n";
      try {
  ...
  AstaConn.messageId = (length = 8, prefix = "WATSON-MD") => {
...
app.listen(randomPort, () =>
  console.log(
    "WATSON-MD Server listening on http://localhost:" + randomPort + "/ "
  )
);
